...
class CalendarManager {
    ...
    if (metricsGrid) {
      ...
      metricsGrid.innerHTML = `
        ...
        <div style="background: #f5f3ff; border: 1px solid #ddd6fe; border-radius: 8px; padding: 0.75rem; text-align: center;">
          <div style="font-size: 0.72rem; color: #6d28d9; text-transform: uppercase; font-weight: 700;">Equipe L1</div>
  ...
  openConsolidatedTodayModal() {
    ...
    // Obter Escala do L1 (Local)
    const scheduleL1 = this.store.getMonthSchedule(yearMonthKey) || this.store.getSchedule(yearMonthKey);
    const dayDataL1 = scheduleL1?.days?.[todayKey];
    const containerL1 = document.getElementById('consolidated-l1-shifts');
    if (containerL1) {
      containerL1.innerHTML = this.renderConsolidatedDayShifts(dayDataL1, 'L1');
    }

    // Obter Escala do L2
    const containerL2 = document.getElementById('consolidated-l2-shifts');
    if (containerL2) {
      containerL2.innerHTML = this.renderConsolidatedL2DayShifts(today, todayKey);
    }
  ...
  renderConsolidatedL2DayShifts(today, todayKey) {
    ...
    if (dow === 0) {
      l2Html = `
        <div style="background: #ffffff; padding: 0.4rem 0.6rem; border-radius: 6px; border: 1px solid #cbd5e1;">
          ...
          <span class="worker-chip color-red" style="font-size: 0.76rem; padding: 2px 7px;">NATÁLIA</span>
        </div>
        <div style="background: #ffffff; padding: 0.4rem 0.6rem; border-radius: 6px; border: 1px solid #cbd5e1;">
          <strong style="font-size: 0.76rem; color: #0f172a; display: block; margin-bottom: 3px;">2º Turno (Plantonista):</strong>
          <span class="worker-chip color-red" style="font-size: 0.76rem; padding: 2px 7px;">CARLOS</span>
        </div>
        <div style="background: #ffffff; padding: 0.4rem 0.6rem; border-radius: 6px; border: 1px solid #cbd5e1;">
          <strong style="font-size: 0.76rem; color: #0f172a; display: block; margin-bottom: 3px;">3º Turno (Noite):</strong>
          <span class="worker-chip color-red" style="font-size: 0.76rem; padding: 2px 7px;">LÍVIA</span>
        </div>
      `;
    } else {
      l2Html = `
        <div style="background: #ffffff; padding: 0.4rem 0.6rem; border-radius: 6px; border: 1px solid #cbd5e1;">
          ...
          <div style="display: flex; flex-wrap: wrap; gap: 4px; align-items: center;">
            <span class="worker-chip color-normal" style="font-size: 0.76rem; padding: 2px 7px;">MATHEUS</span>
            <span class="slot-slash" style="color: #64748b; font-weight: 700; margin: 0 4px; align-self: center;">/</span>
            <span class="worker-chip color-normal" style="font-size: 0.76rem; padding: 2px 7px;">SALETE</span>
          </div>
        </div>
        <div style="background: #ffffff; padding: 0.4rem 0.6rem; border-radius: 6px; border: 1px solid #cbd5e1;">
          <strong style="font-size: 0.76rem; color: #0f172a; display: block; margin-bottom: 3px;">3º Turno (Noite):</strong>
          <div style="display: flex; flex-wrap: wrap; gap: 4px; align-items: center;">
            <span class="worker-chip color-normal" style="font-size: 0.76rem; padding: 2px 7px;">LÍVIA</span>
            <span class="slot-slash" style="color: #64748b; font-weight: 700; margin: 0 4px; align-self: center;">/</span>
            <span class="worker-chip color-normal" style="font-size: 0.76rem; padding: 2px 7px;">NATÁLIA</span>
          </div>
        </div>
      `;
    }
    ...
    if (dayData && dayData.slots) {
      ...
      dayData.slots.forEach((s, idx) => {
        ...
        if (workers.length === 0) {
          ...
          l1Text += `  • *${shiftLabels[idx]}*: Sem escala\n`;
        } else {
          const names = workers.map(w => w.employeeName + (w.color === 'GREEN' ? ' (Folga)' : '')).join(' / ');
          l1Text += `  • *${shiftLabels[idx]}*: ${names}\n`;
        }
      });
    } else {
      l1Text = '  • Escala em aberto\n';
    }
